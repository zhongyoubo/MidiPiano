...
import com.sky.musiclearn.synth.MidiEventScheduler;
import com.sky.musiclearn.synth.MidiFramer;
...
public class SynthEngine extends MidiReceiver {
    ...
    public AudioTrack getAudioTrack(){
        return  mAudioOutput.getAudioTrack();
    }
    ...
    private class MyReceiver extends MidiReceiver {
        ...
        public void onSend(byte[] data, int offset, int count, long timestamp)
            ...
            Debugger.i(TAG , "command == "+command);
            Debugger.i(TAG , "channel == "+channel);
...
//                Debugger.i(TAG , "note off  "+data[1]);
//                Debugger.i(TAG , "note off "+data[2]);
                break;
            case MidiConstants.STATUS_NOTE_ON:
                noteOn(channel, data[1], data[2]);
//                Debugger.i(TAG , "note on  "+data[1]);
//                Debugger.i(TAG , "note on "+data[2]);
                break;
