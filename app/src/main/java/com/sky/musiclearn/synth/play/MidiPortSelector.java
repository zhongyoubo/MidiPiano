...
import com.sky.musiclearn.synth.MidiConstants;
import com.sky.musiclearn.synth.MidiPortWrapper;
...
public abstract class MidiPortSelector extends DeviceCallback {
    private int mType = MidiDeviceInfo.PortInfo.TYPE_INPUT;  //默认的设备的输入类型为输入
   // protected ArrayAdapter<MidiPortWrapper> mAdapter;
    ...
    protected MidiManager mMidiManager; //传递过来的midi设备管理器
    protected Activity mActivity;//传递过来的activity 用来查找布局中的控件
    private MidiPortWrapper mCurrentWrapper; //当前设备的信息包装  MidiDeviceInfo 以及portIndex
    private  Button mButton;//用于连接设备
            ...
            public boolean onKey(View v, int keyCode, KeyEvent event) {
                ...
                switch (keyCode){
                    case KeyEvent.KEYCODE_DPAD_CENTER:
                        Debugger.i(MidiConstants.TAG,"connectButton is clicked center");
                        onPortSelected(mCurrentWrapper);//此方法为抽象方法会调用子类 MidiOutputPortConnectionSelector的onPortSelected方法
                        break;
                    default:
                        break;
                }
